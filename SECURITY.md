...
If you discover a security vulnerability in jarvis, please report it by:
...
chmod 600 ~/.jarvis/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.jarvis/config.json` with file permissions set to `0600`
...
- ✅ Run jarvis with a dedicated user account
...
- Keep authentication data in `~/.jarvis/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for jarvis and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.jarvis
   chmod 600 ~/.jarvis/config.json
   chmod 700 ~/.jarvis/whatsapp-auth
   ```
...
- **Chat history is stored locally** - protect the `~/.jarvis` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.jarvis/logs/jarvis.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/jarvis/security/advisories
- Release Notes: https://github.com/HKUDS/jarvis/releases
