...
import jarvis.channels.matrix as matrix_module
...
from jarvis.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("jarvis.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "jarvis.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("jarvis.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "jarvis.channels.matrix.asyncio.create_task", _fake_create_task
    )
