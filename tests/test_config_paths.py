...
def test_runtime_dirs_follow_config_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("jarvis.config.paths.get_config_path", lambda: config_file)
...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".jarvis" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".jarvis" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".jarvis" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".jarvis" / "workspace"
